...
async def on_logout(sid):
    ...
    if username and username in user_map:
        ...
        del sid_map[sid]
        await sio.emit("user_left", {"username": username})  # va a tutti, incluso chi si disconnette
...
async def disconnect(sid):
    ...
    username = sid_map.get(sid)          # <<< PRIMA prende lo username
    if username and username in user_map:
        del user_map[username]
        del sid_map[sid]
        print("USER_LEFT:", username)
