...
def ensure_connected():
    if not sio.connected:  # attributo previsto dalla libreria
        sio.connect(SERVER)
...
class User():
    ...
    def register_user(self):
        ...
        res = sio.call("register_user", user)
        print("CLIENT register_user res:", res)
        if res.get("ok"):
            # salva la lista utenti per la SelectScreen
            self.initial_users = res.get("users", [])
            return True
        return False
