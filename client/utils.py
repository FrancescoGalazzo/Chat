...
def HEADER(pk, pn, n):
    pk_bytes = pk.public_key().public_bytes_raw()

    pn_bytes = (pn.to_bytes((pn.bit_length() + 7) // 8 or 1, byteorder="big")
                if pn != 0 else b"\x00")
    n_bytes = (n.to_bytes((n.bit_length() + 7) // 8 or 1, byteorder="big")
               if n != 0 else b"\x00")

    return Header(pk_bytes, pn_bytes, n_bytes)
...
def RatchetDecrypt(state, header, ciphertext, AD):
    ...
    # pn come int (big endian)
    pn_int = int.from_bytes(header.pn, byteorder="big")
    if x25519.X25519PublicKey.from_public_bytes(header.dh) != state["DHr"]:
        SkipMessageKeys(state, pn_int)
        DHRatchet(state, header)

    # n come int (big endian)
    n_int = int.from_bytes(header.n, byteorder="big")
